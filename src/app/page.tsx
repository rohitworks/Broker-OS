...
const foundation = [
  ...
  ["Data and auth", "Supabase boundaries configured"],
  ["Private media", "Cloudflare R2 boundary configured"],
  ["Deployment", "Standalone Docker image and health check"],
] as const;
    ...
    <main className="min-h-screen px-5 py-8 sm:px-10 lg:px-16">
      <section className="mx-auto max-w-6xl overflow-hidden rounded-3xl bg-white shadow-[0_18px_70px_rgba(31,50,38,0.10)]">
        <header className="flex items-center justify-between border-b border-black/8 px-6 py-5 sm:px-9">
          ...
          <span className="rounded-full bg-green-50 px-3 py-1 text-xs font-semibold text-green-800">Day 1</span>
        </header>
        <div className="grid gap-10 px-6 py-12 sm:px-9 lg:grid-cols-[1.25fr_1fr] lg:px-12 lg:py-16">
          <div><p className="text-sm font-semibold text-[var(--brand-primary)]">Foundation ready</p><h2 className="mt-3 max-w-2xl text-4xl font-bold leading-tight tracking-tight sm:text-5xl">One dependable operating loop, built from the core outward.</h2><p className="mt-5 max-w-xl text-base leading-7 text-slate-600">The application foundation is configured for a single brokerage deployment while keeping brand, market, contact, locale, and infrastructure values replaceable.</p><div className="mt-8 flex flex-wrap gap-3"><span className="rounded-xl bg-[var(--brand-primary)] px-4 py-2 text-sm font-semibold text-white">{config.defaultLocality}</span><span className="rounded-xl bg-amber-50 px-4 py-2 text-sm font-semibold text-amber-900">{config.currency} · {config.timezone}</span></div></div>
